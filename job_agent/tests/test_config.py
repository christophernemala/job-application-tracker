...
def test_missing_password_raises(monkeypatch):
    ...
    # Prevent .env file from re-loading credentials during reload
    monkeypatch.setattr(cfg, "_load_local_env", lambda: None)
    importlib.reload(cfg)

    # Force-clear password that may have been loaded from .env before monkeypatch
    cfg.NAUKRI_GULF_CREDENTIALS["password"] = ""
